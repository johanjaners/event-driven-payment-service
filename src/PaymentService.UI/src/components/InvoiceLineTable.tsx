      ...
      <table className="w-full text-sm">
          ...
          {lines.map((line) => (
            <tr key={line.id} className="border-b border-border last:border-0">
              ...
              <td className="px-3 py-2 text-right font-medium">
                {formatCurrency(line.unitPrice * line.amount)}
              </td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr className="bg-secondary">
            <td
              colSpan={3}
              className="px-3 py-2 font-bold text-right hidden sm:table-cell"
            >
              Total
            </td>
            <td
              colSpan={2}
              className="px-3 py-2 font-bold text-right sm:hidden"
            >
              Total
            </td>
            <td className="px-3 py-2 font-bold text-right hidden sm:table-cell">
              {formatCurrency(total)}
            </td>
            <td className="px-3 py-2 font-bold text-right sm:hidden">
              {formatCurrency(total)}
            </td>
          </tr>
        </tfoot>
